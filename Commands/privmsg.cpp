...
void Server::handlePrivmsg(int client_fd, const std::string& message)
{
    ...
    for (std::vector<std::string>::iterator target_it = targetList.begin(); target_it != targetList.end(); ++target_it)
    {
        ...
        std::string fullResponse = response + *target_it + " :" + msg + "\r\n";
        ...
        while (channel_it != _channels.end())
        {
            ...
            {
                target = channel_it->second.getName();
                break;
            }
            ++channel_it;
        }
        ...
        {
            int target_fd = getClientFdByName(getLower(target));
            if (target_fd > 0)
            {
                ...
                {
                    std::string fullResponse = response + target_it->getNickname() + " :" + msg + "\r\n";
